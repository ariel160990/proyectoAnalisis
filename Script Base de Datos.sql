...
CREATE TABLE IF NOT EXISTS `Analisis1`.`Caso` (
  ...
  `Tipo_Caso_Id_Tipo` INT NOT NULL COMMENT '',
  PRIMARY KEY (`Id_Caso`)  COMMENT '',
  INDEX `fk_Caso_Usuario_idx` (`Usuario_Usuario` ASC)  COMMENT '',
  INDEX `fk_Caso_Estado1_idx` (`Estado_Id_Estado` ASC)  COMMENT '',
  INDEX `fk_Caso_Tipo_Caso1_idx` (`Tipo_Caso_Id_Tipo` ASC)  COMMENT '',
  CONSTRAINT `fk_Caso_Usuario`
    FOREIGN KEY (`Usuario_Usuario`)
    REFERENCES `Analisis1`.`Usuario` (`Usuario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Caso_Estado1`
    FOREIGN KEY (`Estado_Id_Estado`)
    REFERENCES `Analisis1`.`Estado` (`Id_Estado`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Caso_Tipo_Caso1`
    FOREIGN KEY (`Tipo_Caso_Id_Tipo`)
    REFERENCES `Analisis1`.`Tipo_Caso` (`Id_Tipo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
